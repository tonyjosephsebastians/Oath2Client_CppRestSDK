...
//
static const utility::string_t s_dropbox_key = L"";
static const utility::string_t s_dropbox_secret = L"";
        ...
        : m_listener(new http_listener(listen_uri)), m_config(config)
    {
        m_listener->support([this](http::http_request request) -> void {
            if (request.request_uri().path() == U("/") && !request.request_uri().query().empty())
            {
                ...
                request.reply(status_codes::OK, U("Please Return to your application"));
...
public:
    dropbox_session_sample()
        : oauth2_session_sample(U("Dropbox"),
                                ...
                                U("https://api.dropbox.com/oauth2/token"),
...
protected:
    ...
    {
        http_client api(U("https://api.dropbox.com/2/users/get_current_account"), m_http_config);
        ucout << "Requesting account information:" << std::endl;
     ucout <<"Information: "<<  api.request(methods::POST,L"").get().extract_json().get();
